...
void GRMHD::IntegralsWriter::mapQuantities(
	...
	const double NumberOfLagrangePointsPerAxis = GRMHD::AbstractGRMHDSolver_ADERDG::Order + 1;
	//const double NumberOfUnknownsPerGridPoint = nVar;
	
	// Gauss-Legendre weights from pos argument
	double wx = kernels::gaussLegendreWeights[GRMHD::AbstractGRMHDSolver_ADERDG::Order][pos[0]];
	double wy = kernels::gaussLegendreWeights[GRMHD::AbstractGRMHDSolver_ADERDG::Order][pos[1]];
	double wz = 1;
	#ifdef Dim3
	wz = kernels::gaussLegendreWeights[GRMHD::AbstractGRMHDSolver_ADERDG::Order][pos[2]];
	#endif
