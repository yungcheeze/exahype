...
parser.add_argument('-table',required=True,help="Tables that are to be read.")
...
xdata    = runtimeParser.readColumnFromTable(args.table,0)
maxLevel = runtimeParser.readColumnFromTable(args.table,1)

for adap in args.adapter:
  totalTime    = runtimeParser.readColumnFromTable(args.table, runtimeParser.getAdapterRuntimeColumnFromTable(args.table,adap) )
  count        = runtimeParser.readColumnFromTable(args.table, runtimeParser.getAdapterCountColumnFromTable(args.table,adap) )
  ydata = []
  for i in range(0,len(totalTime)):
    ydata.append( totalTime[i]/(maxLevel[i]**2)/count[i])
  pylab.plot(xdata,ydata,label=adap,markersize=4,marker='',markevery=1,lw=1.2,linestyle='dashed',color='grey')
