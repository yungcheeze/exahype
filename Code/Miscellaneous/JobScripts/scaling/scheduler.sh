...
printf "SCHEDULER_EXECUTABLE=${SCHEDULER_EXECUTABLE}\n"
...
printf "SCHEDULER_MPI_PROCESSES="
for n in "${SCHEDULER_MPI_PROCESSES[@]}"; do printf "$n,"; done
printf "\n"
printf "SCHEDULER_CORES="
for t in "${SCHEDULER_CORES[@]}"; do printf "$t," ; done
printf "\n"
...
# RUN THE EXPERIMENTS
###############################################################################
for n in "${SCHEDULER_MPI_PROCESSES[@]}";
do
  ...
  do
    ...
    do  
      ...
      ${SCHEDULER_SCRIPT} ${SCHEDULER_PROJECT_DIR} ${SCHEDULER_EXECUTABLE} ${SCHEDULER_SPEC_FILE}_$t ${OUTPUT_FILE}
      printf "done!\n"
    done
