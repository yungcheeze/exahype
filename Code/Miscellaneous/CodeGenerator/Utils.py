...
def generateScatter(i_nVar: int, i_chunkSize: int) -> str:
    ...
    if(i_architecture == 'hsw' or i_architecture == 'snb'):
        ...
        for it in range(0, l_iters):
            # aligned load
            l_code = l_code + 'v1 = _mm256_load_pd(&in_buf['+str(0*l_offset+i_simdWidth*it)+']);\n'
            l_code = l_code + 'v2 = _mm256_load_pd(&in_buf['+str(1*l_offset+i_simdWidth*it)+']);\n'
            l_code = l_code + 'v3 = _mm256_load_pd(&in_buf['+str(2*l_offset+i_simdWidth*it)+']);\n'
            l_code = l_code + 'v4 = _mm256_load_pd(&in_buf['+str(3*l_offset+i_simdWidth*it)+']);\n'
            # vunpckhpd, vunpcklpd
        ...
        if(l_remainder > 0):
            ...
            l_startRemainder = l_iters * i_simdWidth
    ...
    if(i_architecture == 'wsm'):
        ...
        for it in range(0, l_iters):
            # aligned load
            l_code = l_code + 'v1 = _mm_load_pd(&in_buf['+str(0*l_offset+i_simdWidth*it)+']);\n'
            l_code = l_code + 'v2 = _mm_load_pd(&in_buf['+str(1*l_offset+i_simdWidth*it)+']);\n'
            # unpcklpd
