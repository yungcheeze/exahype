  ...
  for (int m1 = 0; m1 < basisSize; ++m1) {
    for (int ivar = 0; ivar < numberOfVariables; ++ivar) {
      ...
      for (int n1 = 0; n1 < basisSize; ++n1) {
        ...
        lFhbndFine[mDofStartIndex+ivar]
                          += lFhbndCoarse[nDofStartIndex + ivar] *
                          kernels::fineGridProjector1d[basisSize-1][subfaceIndex[0]][n1][m1]; // f_n * Pi_nm
      }
    }
  }
}
      ...
      for (int n1 = 0; n1 < basisSize; ++n1) {
        ...
        lQhbndCoarse[mDofStartIndex+ivar]
                          += kernels::gaussLegendreWeights[basisSize-1][n1] *
                             ...
                             kernels::gaussLegendreWeights[basisSize-1][m1] / 3;

        lFhbndCoarse[mDofStartIndex+ivar]
                          += kernels::gaussLegendreWeights[basisSize-1][n1] *
                             kernels::fineGridProjector1d[basisSize-1][subfaceIndex[0]][m1][n1] *
        ...
        for (int n2 = 0; n2 < basisSize; ++n2) {
          for (int n1 = 0; n1 < basisSize; ++n1) {
            ...
            luhCoarse[mDofStartIndex+ivar] +=
                ...
                kernels::gaussLegendreWeights[basisSize-1][m2] / 9; // 9=3^DIMENSIONS
          }
        }
      }
    }
