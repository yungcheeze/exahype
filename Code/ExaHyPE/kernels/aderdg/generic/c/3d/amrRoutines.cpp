        ...
        for (int n2 = 0; n2 < basisSize; ++n2) {
          for (int n1 = 0; n1 < basisSize; ++n1) {
            ...
            lQhbndFine[mDofStartIndex+ivar] +=
                ...
                kernels::fineGridProjector1d[basisSize-1][subfaceIndex[1]][n2][m2];// q_n * Pi_nm
          }
        }
      }
    }
...
void kernels::aderdg::generic::c::faceUnknownsProlongation(double* lQhbndFine,
  ...
  for (int l = 1; l < levelDelta+1; ++l) {
    ...
    memset(workPointerQhbnd1, 0, basisSize*basisSize*numberOfVariables*sizeof(double));
    memset(workPointerFhbnd1, 0, basisSize*basisSize*numberOfVariables*sizeof(double));
...
void kernels::aderdg::generic::c::faceUnknownsRestriction(double* lQhbndCoarse,
  ...
  accumulate(lQhbndCoarse, workPointerQhbnd2, basisSize*basisSize*numberOfVariables);
  accumulate(lFhbndCoarse, workPointerFhbnd2, basisSize*basisSize*numberOfVariables);
            ...
            for (int n2 = 0; n2 < basisSize; ++n2) {
              for (int n1 = 0; n1 < basisSize; ++n1) {
                ...
                luhFine[mDofStartIndex+ivar]
                        ...
                        kernels::fineGridProjector1d[basisSize-1][subcellIndex[2]][n3][m3]; // \sum q_n * P_nm
              }
            }
          }
        }
...
void kernels::aderdg::generic::c::volumeUnknownsProlongation(double* luhFine,
  ...
  for (int l = 1; l < levelDelta+1; ++l) {
    ...
    // Zero the values of 'workPointer1' since we compute a sum for each entry.
    memset(workPointerUh1, 0, basisSize*basisSize*basisSize*numberOfVariables*sizeof(double));
            ...
            for (int n2 = 0; n2 < basisSize; ++n2) {
              for (int n1 = 0; n1 < basisSize; ++n1) {
                ...
                luhCoarse[mDofStartIndex+ivar] +=
                    ...
                    kernels::gaussLegendreWeights[basisSize-1][m3] / 27.0; // 9=3^DIMENSIONS
              }
            }
          }
        }
...
void kernels::aderdg::generic::c::volumeUnknownsRestriction(
  ...
  // Add to coarse grid degrees of freedom:
  accumulate(luhCoarse,workPointerUh2,basisSize*basisSize*basisSize*numberOfVariables);
