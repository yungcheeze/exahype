...
exahype::plotters::ADERDG2BinaryVTK::ADERDG2BinaryVTK(const std::string& filename, int order, int unknowns):
  ...
  for (int i=0; i<unknowns; i++) {
    ...
    identifier << "Q" << i;
    _vertexDataWriter.push_back( _patchWriter->createVertexDataWriter( identifier.str(), 1 ) );
  }
}
...
exahype::plotters::ADERDG2BinaryVTK::~ADERDG2BinaryVTK() {
  ...
  for (
    std::vector< tarch::plotter::griddata::Writer::VertexDataWriter* >::iterator p = _vertexDataWriter.begin();
