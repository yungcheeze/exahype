...
void kernels::initSolvers(exahype::Parser& parser, std::vector<std::string>& cmdlineargs) {
  ...
  {
  solver = new MHD::MHDSolver_ADERDG(parser.getMaximumMeshSize(0), parser.getTimeStepping(0), cmdlineargs);
  }
  std::unique_ptr<exahype::solvers::ADERDGSolver> aderdgSolver(static_cast<exahype::solvers::ADERDGSolver*>(solver));
  {
  solver = new MHD::MHDSolver_FV(2*(aderdgSolver->getNodesPerCoordinateAxis()-1)+1, parser.getMaximumMeshSize(0), parser.getTimeStepping(0));
  }
  ...
  exahype::plotters::RegisteredPlotters.push_back( new exahype::plotters::Plotter(0,0,parser,new MHD::MHDSolver_Plotter0(  *static_cast<exahype::solvers::LimitingADERDGSolver*>(exahype::solvers::RegisteredSolvers[0])) ));
