...
void Elastic::MyElasticSolver::adjustedSolutionValues(const double* const x,const double w,const double t,const double dt,double* Q) {
  ...
  if (tarch::la::equals(t, 0.0)) {
    if(true){
      ...
      Q[0] = 0.0; // std::exp(-((x[0]-x0)*(x[0]-x0) + (x[1]-y0)*(x[1]-y0) + (x[2]-z0)*(x[2]-z0) ) /(1.0 * 1)) *10.0;
      Q[1] = 0.0; //std::exp(-((x[0]-x0)*(x[0]-x0) + (x[1]-y0)*(x[1]-y0) + (x[2]-z0)*(x[2]-z0) ) /(1 * 1)) *10.0;
      Q[2] = 0.0; //std::exp(-((x[0]-x0)*(x[0]-x0) + (x[1]-y0)*(x[1]-y0) + (x[2]-z0)*(x[2]-z0) ) /(1 * 1)) *10.0;
      Q[3] = 0.0;
      Q[4] = 0.0;
      Q[5] = 0.0;
      Q[6] = 0.0;
...
void  Elastic::MyElasticSolver::pointSources(double *luh,const tarch::la::Vector<DIMENSIONS,double>& center,const tarch::la::Vector<DIMENSIONS,double>& dx,double t,double dt, int numberOfVariables, int numberOfParameters, int _order) {
  ...
  /** Addition of point source terms **/
   /* if (
    ...
    if(xRef(0) >= 0. && xRef(1) >= 0.  && xRef(2) >= 0. && xRef(0) <= 1.0 && xRef(1) <= 1.0 && xRef(2) <= 1.0) {
      //
      	double value = 0.0;
	double sourceInt = 0.0;
	sourceInt = ((1.0-(1.0+t/0.1)*(std::exp(-t/0.1)))-(1.0-(1.0+(t-dt)/0.1)*(std::exp(-(t-dt)/0.1)))); //1.;
	//sourceInt = 0.5*(t/(0.1*0.1)*(std::exp(-t/0.1)) + (t-dt)/(0.1*0.1)*(std::exp(-(t-dt)/0.1)));
	if (tarch::la::equals(t, 0.0)){
	  sourceInt = 0.0;
	...
	//exit(-1);
      for (int unknown=0; unknown < numberOfVariables /*numberOfParameters*/; unknown++) {
