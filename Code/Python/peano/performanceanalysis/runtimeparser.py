...
.. module:: runtimeparser
  ...
  :synopsis: Provides functions to read the average CPU and user times of a set of Peano output files.
...
def parse_all_adapter_times(rootdir,prefix,process_counts,thread_counts,n_runs=1,cc='icpc',mode='tbb',per_iteration=False):
    ...
    Reads multiple Peano output files with the naming pattern
       '<prefix>_n<#MPI proc.>_t<#threads/MPI proc.>_r<#runs>_<cc>_<mode>.txt'

    and computes the average, minimum, and maximum runtimes.
...
def parse_adapter_times(file_path,per_iteration=False):
    """
    Reads a single Peano output file and parses the user time spent in 'adapter'.
