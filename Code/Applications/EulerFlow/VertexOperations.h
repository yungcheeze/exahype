...
class exahype::VertexOperations {
 private:
  /**
   * One should not instantiate this class as it is a collection of static
   * operations.
   */
  VertexOperations();

 public:
  static tarch::la::Vector<TWO_POWER_D, int> readCellDescriptionsIndex(
      const Vertex& vertex);

  static tarch::la::Vector<TWO_POWER_D_TIMES_TWO_POWER_D, int>
  readCellDescriptionsIndex(const peano::grid::VertexEnumerator& enumerator,
                            const Vertex* const vertices);

  static void writeCellDescriptionsIndex(
