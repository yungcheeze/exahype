...
#ifndef EXAHYPE_ADAPTERS_PatchInitialisationAndExport2MultiscaleLinkedCell_1_H_
#define EXAHYPE_ADAPTERS_PatchInitialisationAndExport2MultiscaleLinkedCell_1_H_
...
namespace exahype {
namespace adapters {
class PatchInitialisationAndExport2MultiscaleLinkedCell_1;
}
}
 ...
 *
 * CellDescriptionsIndex   Name of the index used for the cell indices within
 *the vertex and
 *          the cell
 *
 ...
 */
class exahype::adapters::PatchInitialisationAndExport2MultiscaleLinkedCell_1 {
 public:
  ...
  PatchInitialisationAndExport2MultiscaleLinkedCell_1(
      const PatchInitialisationAndExport2MultiscaleLinkedCell_1& masterThread);
#endif

  virtual ~PatchInitialisationAndExport2MultiscaleLinkedCell_1();

#if defined(SharedMemoryParallelisation)
  void mergeWithWorkerThread(
      const PatchInitialisationAndExport2MultiscaleLinkedCell_1& workerThread);
#endif
   ...
   * order to fill the ghostlayers of the cellDescriptiones appropriately, the
   *adjacent
   ...
   *
   * !!! The algorithm
   * It gets a hanging vertex and performs a loop over the $2^d$
   *adjacent-cellDescription-
   * indices.
   ...
   *  - If the fine grid position of the hanging vertex in dimension $d$ is 0
   *set
   ...
   *    the adjacent-cellDescription-index $k$ that has to be set currently.
   *$k(d)$ can
   ...
   *    matter since the appropriate adjacent-cellDescription-indices of the to
   *coarse
   *    grid vertices have to be the same, since they are pointing to the same
   *    adjacent cell.
   * The determination of the correct adjacent-cellDescription-index of the
   *coarse grid
   * vertex (coarseGridVertexAdjacentCellDescriptionIndex) is done in a similar
   *way. So,
   ...
   *    just copy the adjacent-cellDescription-index. Therefore, if the fine
   *grid position
   *    of the hanging vertex in dimension $d$ is equal to 0 or to 3, we set
   *    $coarseGridVertexAdjacentCellDescriptionIndex(d)$ to $k(d)$.
   *  - Otherwise, we just set $coarseGridVertexAdjacentCellDescriptionIndex(d)$
   *to the
   *    inverted $k(d)$. I.e. if $k(d) = 0$ we set
   *    $coarseGridVertexAdjacentCellDescriptionIndex(d)$ to 1 and the other way
   *around.
   *
   */
