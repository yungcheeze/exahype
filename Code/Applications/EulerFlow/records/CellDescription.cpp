#include "EulerFlow/records/CellDescription.h"

exahype::records::CellDescription::PersistentRecords::PersistentRecords() {}

exahype::records::CellDescription::PersistentRecords::PersistentRecords(
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& spaceTimePredictor,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& spaceTimeVolumeFlux,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& solution,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& update,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& predictor,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& volumeFlux,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>&
        extrapolatedPredictor,
    const tarch::la::Vector<EXAHYPE_PATCH_SIZE_TOTAL, int>& fluctuation,
...
exahype::records::CellDescription::CellDescription(
    ...
    const int& level, const tarch::la::Vector<DIMENSIONS, double>& offset,
    const tarch::la::Vector<DIMENSIONS, double>& size)
    : _persistentRecords(spaceTimePredictor, spaceTimeVolumeFlux, solution,
                         update, predictor, volumeFlux, extrapolatedPredictor,
                         fluctuation, level, offset, size) {}
...
exahype::records::CellDescription::PersistentRecords
exahype::records::CellDescription::getPersistentRecords() const {
  return _persistentRecords;
}

exahype::records::CellDescriptionPacked
exahype::records::CellDescription::convert() const {
...
#ifdef Parallel
tarch::logging::Log exahype::records::CellDescription::_log(
...
void exahype::records::CellDescription::initDatatype() {
  {
    ...
    MPI_Aint base;
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(&(dummyCellDescription[0]))),
        &base);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]
                          ._persistentRecords._spaceTimePredictor[0]))),
                &disp[0]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]
                          ._persistentRecords._spaceTimeVolumeFlux[0]))),
                &disp[1]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescription[0]._persistentRecords._solution[0]))),
        &disp[2]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]._persistentRecords._update[0]))),
                &disp[3]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescription[0]._persistentRecords._predictor[0]))),
        &disp[4]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescription[0]._persistentRecords._volumeFlux[0]))),
        &disp[5]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]
                          ._persistentRecords._extrapolatedPredictor[0]))),
                &disp[6]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescription[0]._persistentRecords._fluctuation[0]))),
        &disp[7]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]._persistentRecords._level))),
                &disp[8]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]._persistentRecords._offset[0]))),
                &disp[9]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescription[0]._persistentRecords._size[0]))),
                &disp[10]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &dummyCellDescription[1]
                         ._persistentRecords._spaceTimePredictor[0])),
                &disp[11]);
...
void exahype::records::CellDescription::send(
    ...
    while (!flag) {
      ...
      if (result != MPI_SUCCESS) {
        ...
        msg << "testing for finished send task for "
               "exahype::records::CellDescription " << toString()
      ...
      if (tarch::parallel::Node::getInstance().isTimeOutWarningEnabled() &&
        ...
        tarch::parallel::Node::getInstance().writeTimeOutWarning(
            "exahype::records::CellDescription", "send(int)", destination, tag,
            1);
...
void exahype::records::CellDescription::receive(
  ...
  if (communicateSleep < 0) {
    ...
    if (result != MPI_SUCCESS) {
      std::ostringstream msg;
      msg << "failed to start to receive exahype::records::CellDescription "
             "from node " << source << ": "
          << tarch::parallel::MPIReturnValueToString(result);
    ...
    while (!flag) {
      ...
      if (result != MPI_SUCCESS) {
        ...
        msg << "testing for finished receive task for "
               "exahype::records::CellDescription failed: "
            << tarch::parallel::MPIReturnValueToString(result);
        _log.error("receive(int)", msg.str());
      }
      ...
      if (tarch::parallel::Node::getInstance().isTimeOutWarningEnabled() &&
          (clock() > timeOutWarning) && (!triggeredTimeoutWarning)) {
        tarch::parallel::Node::getInstance().writeTimeOutWarning(
            "exahype::records::CellDescription", "receive(int)", source, tag,
            1);
        triggeredTimeoutWarning = true;
      }
      if (tarch::parallel::Node::getInstance().isTimeOutDeadlockEnabled() &&
...
bool exahype::records::CellDescription::isMessageInQueue(
...
exahype::records::CellDescriptionPacked::PersistentRecords::
    PersistentRecords() {}
...
void exahype::records::CellDescriptionPacked::toString(
...
exahype::records::CellDescriptionPacked::PersistentRecords
exahype::records::CellDescriptionPacked::getPersistentRecords() const {
  return _persistentRecords;
}

exahype::records::CellDescription
exahype::records::CellDescriptionPacked::convert() const {
  return CellDescription(getSpaceTimePredictor(), getSpaceTimeVolumeFlux(),
                         getSolution(), getUpdate(), getPredictor(),
                         getVolumeFlux(), getExtrapolatedPredictor(),
                         getFluctuation(), getLevel(), getOffset(), getSize());
}

#ifdef Parallel
tarch::logging::Log exahype::records::CellDescriptionPacked::_log(
    "exahype::records::CellDescriptionPacked");
...
void exahype::records::CellDescriptionPacked::initDatatype() {
  {
    ...
    MPI_Address(const_cast<void*>(
                    static_cast<const void*>(&(dummyCellDescriptionPacked[0]))),
                &base);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescriptionPacked[0]
                          ._persistentRecords._spaceTimePredictor[0]))),
                &disp[0]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescriptionPacked[0]
    ...
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._solution[0]))),
        &disp[2]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._update[0]))),
        &disp[3]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._predictor[0]))),
        &disp[4]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescriptionPacked[0]
                          ._persistentRecords._volumeFlux[0]))),
                &disp[5]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescriptionPacked[0]
                          ._persistentRecords._extrapolatedPredictor[0]))),
    ...
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &(dummyCellDescriptionPacked[0]
                          ._persistentRecords._fluctuation[0]))),
                &disp[7]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._level))),
        &disp[8]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._offset[0]))),
        &disp[9]);
    MPI_Address(
        const_cast<void*>(static_cast<const void*>(
            &(dummyCellDescriptionPacked[0]._persistentRecords._size[0]))),
        &disp[10]);
    MPI_Address(const_cast<void*>(static_cast<const void*>(
                    &dummyCellDescriptionPacked[1]
                         ._persistentRecords._spaceTimePredictor[0])),
                &disp[11]);
    ...
    MPI_Type_struct(Attributes, blocklen, disp, subtypes,
                    &CellDescriptionPacked::Datatype);
  ...
  }
  {
    ...
    MPI_Type_struct(Attributes, blocklen, disp, subtypes,
                    &CellDescriptionPacked::FullDatatype);
...
void exahype::records::CellDescriptionPacked::send(
    int destination, int tag, bool exchangeOnlyAttributesMarkedWithParallelise,
    int communicateSleep) {
  if (communicateSleep < 0) {
    const int result = MPI_Send(
    ...
    if (result != MPI_SUCCESS) {
      ...
      msg << "was not able to send message "
             "exahype::records::CellDescriptionPacked " << toString()
          << " to node " << destination << ": "
          << tarch::parallel::MPIReturnValueToString(result);
      _log.error("send(int)", msg.str());
    }
    ...
    while (!flag) {
      ...
      if (tarch::parallel::Node::getInstance().isTimeOutWarningEnabled() &&
          (clock() > timeOutWarning) && (!triggeredTimeoutWarning)) {
        tarch::parallel::Node::getInstance().writeTimeOutWarning(
            "exahype::records::CellDescriptionPacked", "send(int)", destination,
            tag, 1);
        triggeredTimeoutWarning = true;
      }
      if (tarch::parallel::Node::getInstance().isTimeOutDeadlockEnabled() &&
...
void exahype::records::CellDescriptionPacked::receive(
    int source, int tag, bool exchangeOnlyAttributesMarkedWithParallelise,
    int communicateSleep) {
  if (communicateSleep < 0) {
    MPI_Status status;
    ...
    if (result != MPI_SUCCESS) {
      ...
      msg << "failed to start to receive "
             "exahype::records::CellDescriptionPacked from node " << source
          << ": " << tarch::parallel::MPIReturnValueToString(result);
      _log.error("receive(int)", msg.str());
    }
    ...
    while (!flag) {
      ...
      if (result != MPI_SUCCESS) {
        std::ostringstream msg;
        msg << "testing for finished receive task for "
               "exahype::records::CellDescriptionPacked failed: "
            << tarch::parallel::MPIReturnValueToString(result);
        _log.error("receive(int)", msg.str());
      }
      ...
      if (tarch::parallel::Node::getInstance().isTimeOutWarningEnabled() &&
          (clock() > timeOutWarning) && (!triggeredTimeoutWarning)) {
        tarch::parallel::Node::getInstance().writeTimeOutWarning(
            "exahype::records::CellDescriptionPacked", "receive(int)", source,
            tag, 1);
        triggeredTimeoutWarning = true;
      }
      if (tarch::parallel::Node::getInstance().isTimeOutDeadlockEnabled() &&
...
bool exahype::records::CellDescriptionPacked::isMessageInQueue(
    int tag, bool exchangeOnlyAttributesMarkedWithParallelise) {
  MPI_Status status;
  int flag = 0;
  MPI_Iprobe(MPI_ANY_SOURCE, tag,
