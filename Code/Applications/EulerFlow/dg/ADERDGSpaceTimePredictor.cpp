...
void exahype::dg::spaceTimePredictor<3>(
    double *restrict /*local*/ lQi, double *restrict /*local*/ lFi,
    const double *restrict const /*in*/ luh, double *restrict /*out*/ lQhi,
    double *restrict /*out*/ lFhi, double *restrict /*out*/ lQhbnd,
    double *restrict /*out*/ lFhbnd, double *restrict /*local*/ rhs0,
    double *restrict /*local*/ rhs, double *restrict /*local*/ tmp,
    const double *restrict const /*in*/ dx, const double /*in*/ dt) {
      ...
      for (int ll = 0; ll < basisSize; ll++) {  // loop over dof
        ...
        for (int ivar = 0; ivar < nvar; ivar++) {
          ...
          rhs0[spaceTimeDofStartIndex + ivar] =
              quad::gaussLegendreWeights[ii] * quad::gaussLegendreWeights[jj] *
              dg::F0[ll] * luh[dofStartIndex + ivar];
        }
      }
    }
  }
  ...
  for (int iter = 1; iter < basisSize + 1; iter++) {
    ...
    for (int ll = 0; ll < basisSize; ll++) {    // loop over dof (time)
      ...
      for (int ii = 0; ii < basisSize; ii++) {  // loop over dof
        for (int jj = 0; jj < basisSize; jj++) {
          ...
          double weight =
              quad::gaussLegendreWeights[ll] * quad::gaussLegendreWeights[jj];
      ...
      for (int ii = 0; ii < basisSize; ii++) {  // loop over dof
        for (int jj = 0; jj < basisSize; jj++) {
          ...
          double weight =
              quad::gaussLegendreWeights[ll] * quad::gaussLegendreWeights[ii];
    ...
    for (int ii = 0; ii < basisSize; ii++) {  // loop over dof
      for (int jj = 0; jj < basisSize; jj++) {
        ...
        double iWeight = 1. / (quad::gaussLegendreWeights[ii] *
        ...
        for (int ll = 0; ll < basisSize; ll++) {  // loop over dof
          ...
          for (int nn = 0; nn < basisSize; nn++) {
            ...
            for (int ivar = 0; ivar < nvar; ivar++) {
              tmp[ivar + nvar * ll] += iWeight * dg::iK1[ll][nn] *
      ...
      for (int ll = 0; ll < basisSize; ll++) {  // loop over dof
        ...
        for (int ivar = 0; ivar < nvar; ivar++) {
          ...
          lFhi_x[dofStartIndex + ivar] +=
              weight * f[ivar];  // lFhi_x[nDOF_y][nDOF_x][nVar]
          // lFhi_y[dofStartIndex+ivar] += weight * g[ivar]; //
          // lFhi_y[nDOF_y][nDOF_x][nVar] (former version)
          lFhi_y[(ii + basisSize * jj) * nvar + ivar] +=
    ...
    for (int mm = 0; mm < basisSize; mm++) {  // loop over dof
      ...
      for (int ivar = 0; ivar < nvar; ivar++) {
        lQhbnd[dofStartIndexL + ivar] +=
            dg::FLCoeff[mm] * Q[ivar];  // lQhbnd(Facei,nDOF,nVar)
        lQhbnd[dofStartIndexR + ivar] +=
            dg::FRCoeff[mm] * Q[ivar];  // lQhbnd(Facei,nDOF,nVar)
  ...
  for (int ii = 0; ii < basisSize; ii++) {
    ...
    for (int mm = 0; mm < basisSize; mm++) {
      ...
      for (int ivar = 0; ivar < nvar; ivar++) {
        ...
        // lFhbnd[dofStartIndexL+ivar] += dg::FLCoeff[mm] *
        // lFhi_y[mmDofStartIndex+ivar]; // lFhi_y * FLCoeff
        // lFhbnd[dofStartIndexR+ivar] += dg::FRCoeff[mm] *
