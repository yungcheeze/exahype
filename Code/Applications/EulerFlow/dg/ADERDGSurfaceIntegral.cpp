...
// 3D
void exahype::dg::surfaceIntegral(double *restrict /*inout*/ lduh,
...
void exahype::dg::surfaceIntegral(double *restrict /*inout*/ lduh,
  ...
  for (int jj = 0; jj < basisSize; jj++) {
    ...
    for (int mm = 0; mm < basisSize; mm++) {
      for (int ivar = 0; ivar < nvar; ivar++) {
        lduh3D[jj][mm][ivar] -=
            weight / dx[0] * (dg::FRCoeff[mm] * FRight[dofStartIndex + ivar] -
                              dg::FLCoeff[mm] * FLeft[dofStartIndex + ivar]);
      }
    }
  }
  ...
  for (int jj = 0; jj < basisSize; jj++) {
    ...
    for (int mm = 0; mm < basisSize; mm++) {
      for (int ivar = 0; ivar < nvar; ivar++) {
        lduh3D[mm][jj][ivar] -=
            weight / dx[1] * (dg::FRCoeff[mm] * FBack[dofStartIndex + ivar] -
                              dg::FLCoeff[mm] * FFront[dofStartIndex + ivar]);
      }
    }
  }
