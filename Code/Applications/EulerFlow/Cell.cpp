...
void exahype::Cell::initCellWithDefaultValues() {
  _cellData.setCellDescriptionsIndex(
      multiscalelinkedcell::HangingVertexBookkeeper::InvalidAdjacencyIndex);
}
...
void exahype::Cell::initCellInComputeTree(
    const int level, const tarch::la::Vector<DIMENSIONS, double> size,
    const int numberOfPDEs, const int order, const int numberOfVariables) {
  const int indexOfCellDescriptions =
      CellDescriptionHeap::getInstance().createData(numberOfPDEs);
  ...
  for (int pde = 0; pde < numberOfPDEs; pde++) {
    ...
    //  @todo: consider to precompute these values/to use a lookup table at this
    //  point
    int numberOfSpaceTimeDof =
    ...
    int numberOfDofOnFace = DIMENSIONS_TIMES_TWO * EXAHYPE_NVARS *
    ...
    for (int i = 0; i < EXAHYPE_PATCH_SIZE_X + 2; i++) {  // loop over patches
      for (int j = 0; j < EXAHYPE_PATCH_SIZE_Y + 2; j++) {
        ...
        cellDescriptionForPde.setSpaceTimePredictor(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);
        cellDescriptionForPde.setSpaceTimeVolumeFlux(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);

        // Initialize patch volume DoF to invalid value
        cellDescriptionForPde.setSolution(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);
        cellDescriptionForPde.setUpdate(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);
        cellDescriptionForPde.setPredictor(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);
        cellDescriptionForPde.setVolumeFlux(
            patchIndex, multiscalelinkedcell::HangingVertexBookkeeper::
                            InvalidAdjacencyIndex);
        ...
        if (i > 0 || i < EXAHYPE_PATCH_SIZE_X + 1) {
          if (j > 0 || j < EXAHYPE_PATCH_SIZE_Y + 1) {
            ...
            cellDescriptionForPde.setSpaceTimePredictor(
                patchIndex, DataHeap::getInstance().createData(
                                numberOfSpaceTimeDof, numberOfSpaceTimeDof));
            cellDescriptionForPde.setSpaceTimeVolumeFlux(
                patchIndex, DataHeap::getInstance().createData(
                                numberOfSpaceTimeVolumeFluxDof,
                                numberOfSpaceTimeVolumeFluxDof));

            // Allocate volume DoF
            cellDescriptionForPde.setSolution(
                patchIndex,
                DataHeap::getInstance().createData(numberOfDof, numberOfDof));
            cellDescriptionForPde.setUpdate(
                patchIndex,
                DataHeap::getInstance().createData(numberOfDof, numberOfDof));
            cellDescriptionForPde.setPredictor(
            ...
            cellDescriptionForPde.setVolumeFlux(
                patchIndex, DataHeap::getInstance().createData(
                                numberOfVolumeFluxDof, numberOfVolumeFluxDof));
          }
        }

        // Allocate face DoF
        cellDescriptionForPde.setExtrapolatedPredictor(
            patchIndex, DataHeap::getInstance().createData(numberOfDofOnFace,
                                                           numberOfDofOnFace));
        cellDescriptionForPde.setFluctuation(
            patchIndex, DataHeap::getInstance().createData(numberOfDofOnFace,
                                                           numberOfDofOnFace));
      }
    }
