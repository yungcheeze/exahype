...
exahype::VertexOperations::readCellDescriptionsIndex(
  ...
  dfor2(x) tarch::la::slice(
      result, vertices[enumerator(x)]._vertexData.getCellDescriptionsIndex(),
      xScalar * TWO_POWER_D);
  enddforx return result;
}

tarch::la::Vector<TWO_POWER_D, int>
exahype::VertexOperations::readCellDescriptionsIndex(const Vertex& vertex) {
  return vertex._vertexData.getCellDescriptionsIndex();
}

void exahype::VertexOperations::writeCellDescriptionsIndex(
  ...
  vertices[enumerator(x)]._vertexData.setCellDescriptionsIndex(temp);
  enddforx
}

void exahype::VertexOperations::writeCellDescriptionsIndex(
    Vertex& vertex, const tarch::la::Vector<TWO_POWER_D, int>& values) {
  vertex._vertexData.setCellDescriptionsIndex(values);
}
