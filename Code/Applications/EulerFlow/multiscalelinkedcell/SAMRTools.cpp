...
void multiscalelinkedcell::SAMRTools::
    computeCellDescriptionOverlapWithGhostLayer(
...
void multiscalelinkedcell::SAMRTools::
    computeOppositeOffsetFromRelativePositionForSourceImage(
...
#endif
  for (int d = 0; d < DIMENSIONS; d++) {
    assertion2(tarch::la::smallerEquals(leftBottom(d), rightTop(d)), leftBottom,
               rightTop);

    const double numberOfCells = (leftBottom(d) - offset(d)) / dxdy(d);
    //    cellOffset(d) = tarch::la::round( (leftBottom(d)-offset(d)+dxdy(d)) /
    //    dxdy(d)); //-> 3.1
    cellOffset(d) = std::floor(numberOfCells) + haloSize(d);  //-> 2.1
    //    cellOffset(d) = std::ceil( (leftBottom(d)-offset(d)+dxdy(d)) /
    //    dxdy(d)); -> 3.1
  }

  for (int d = 0; d < DIMENSIONS; d++) {
    //    range(d) = std::ceil( continuousRage(d)/dxdy(d) );
    // it should be ceil logically, but then we have rounding effects, i.e.
    // 1.0000000000001 becoming 2.
...
int multiscalelinkedcell::SAMRTools::
    getIndexOfFirstInnerUnknownInCellDescription(
        const tarch::la::Vector<DIMENSIONS, int>& cells,
        const tarch::la::Vector<DIMENSIONS, int>& haloCells) {
  int result = 0;
  int base = 1;
