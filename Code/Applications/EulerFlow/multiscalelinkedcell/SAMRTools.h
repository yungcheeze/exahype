...
class multiscalelinkedcell::SAMRTools {
 public:
  static int getNumberOfCellsPerCellDescription(
      const tarch::la::Vector<DIMENSIONS, int>& cells,
      const tarch::la::Vector<DIMENSIONS, int>& haloCells);
  static int getNumberOfVerticesPerCellDescription(
      const tarch::la::Vector<DIMENSIONS, int>& cells,
      const tarch::la::Vector<DIMENSIONS, int>& haloCells);

  /**
   * Computes the continuous overlap of a cellDescription with the ghost layer.
   *The
   * result is independent of the actual adaptivity structure.
   *
   * @param dxdy Size of one cell of the cellDescription embedded into the
   *spacetree
  ...
  /**
   * This operation is given the continuous overlap of two cellDescriptiones
   * (see
   * computeCellDescriptionOverlapWithGhostLayer()) and then computes the
   * iteration
  ...
  /**
   * This operation is given a cell specified by a cellDescriptiones size and a
   * number
   ...
   * This operation is given a cell specified by a cellDescriptiones size and a
   * number
   * of halo cells. If returns the iteration range in the source cellDescription
   * while
   * it assumes that this one has the same dimension as the local
   * cellDescription.
   */
  ...
  /**
   * If the cellDescription is a huge array, the very first unknown has a
   *certain
   * offset. If the cellDescription size is for example three cells and we have
   *a one
   * cell  halo, then the first real inner unknown has index six. This
   * operation returns that index.
   *
   * @return Index of first real unknown within cellDescription (without ghost
   *cells).
   *
