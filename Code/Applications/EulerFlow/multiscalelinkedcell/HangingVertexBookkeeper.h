...
class multiscalelinkedcell::HangingVertexBookkeeper {
 private:
  ...
  typedef std::map<tarch::la::Vector<DIMENSIONS + 1, double>,
                   HangingVertexIdentifier,
                   tarch::la::VectorCompare<DIMENSIONS + 1> > VertexMap;
 ...
 public:
  ...
  tarch::la::Vector<TWO_POWER_D, int> createVertexLinkMapForBoundaryVertex()
      const;
  ...
  bool holdsVertex(const tarch::la::Vector<DIMENSIONS, double>& x,
                   int level) const;

  bool usedVertexInThisTraversal(const tarch::la::Vector<DIMENSIONS, double>& x,
                                 int level) const;
  ...
  void updateCellIndicesInMergeWithNeighbour(Vertex& vertex);

  void beginIteration();

  /**
   * Remove all those hanging vertex entries that were not used in the previous
   * iteration.
   */
  void endIteration();

  static bool allAdjacencyInformationIsAvailable(
...
#include "EulerFlow/multiscalelinkedcell/HangingVertexBookkeeper.cpph"
