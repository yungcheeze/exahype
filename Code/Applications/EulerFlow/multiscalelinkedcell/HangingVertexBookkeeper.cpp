...
const int multiscalelinkedcell::HangingVertexBookkeeper::InvalidAdjacencyIndex =
    -1;
const int multiscalelinkedcell::HangingVertexBookkeeper::RemoteAdjacencyIndex =
    -2;
const int multiscalelinkedcell::HangingVertexBookkeeper::
    DomainBoundaryAdjacencyIndex = -3;

tarch::logging::Log multiscalelinkedcell::HangingVertexBookkeeper::_log(
    "multiscalelinkedcell::HangingVertexBookkeeper");

tarch::la::Vector<THREE_POWER_D, int>
...
#ifdef Dim2
  assertionEquals1(indices(0 * TWO_POWER_D + 1), indices(1 * TWO_POWER_D + 0),
                   indices);
  assertionEquals1(indices(0 * TWO_POWER_D + 3), indices(3 * TWO_POWER_D + 0),
                   indices);
  assertionEquals1(indices(3 * TWO_POWER_D + 2), indices(2 * TWO_POWER_D + 3),
                   indices);
  assertionEquals1(indices(3 * TWO_POWER_D + 1), indices(1 * TWO_POWER_D + 3),
                   indices);

  assertionEquals1(indices(0 * TWO_POWER_D + 3), indices(1 * TWO_POWER_D + 2),
                   indices);
  assertionEquals1(indices(1 * TWO_POWER_D + 2), indices(3 * TWO_POWER_D + 0),
                   indices);
  assertionEquals1(indices(3 * TWO_POWER_D + 0), indices(2 * TWO_POWER_D + 1),
                   indices);
...
tarch::la::Vector<TWO_POWER_D, int> multiscalelinkedcell::
    HangingVertexBookkeeper::createVertexLinkMapForBoundaryVertex() const {
  return tarch::la::Vector<TWO_POWER_D, int>(DomainBoundaryAdjacencyIndex);
}

bool multiscalelinkedcell::HangingVertexBookkeeper::
    allAdjacencyInformationIsAvailable(
        const tarch::la::Vector<TWO_POWER_D, int>& arg) {
  return !tarch::la::oneEquals(arg, InvalidAdjacencyIndex);
}

bool multiscalelinkedcell::HangingVertexBookkeeper::
    allAdjacencyInformationIsAvailable(
        const tarch::la::Vector<THREE_POWER_D, int>& arg) {
  return !tarch::la::oneEquals(arg, InvalidAdjacencyIndex);
}

bool multiscalelinkedcell::HangingVertexBookkeeper::
    allAdjacencyInformationIsAvailable(
        const tarch::la::Vector<TWO_POWER_D_TIMES_TWO_POWER_D, int>& arg) {
  return !tarch::la::oneEquals(arg, InvalidAdjacencyIndex);
}

tarch::la::Vector<DIMENSIONS + 1, double>
multiscalelinkedcell::HangingVertexBookkeeper::getKey(
    const tarch::la::Vector<DIMENSIONS, double>& x, int level) const {
...
tarch::la::Vector<TWO_POWER_D, int>&
multiscalelinkedcell::HangingVertexBookkeeper::getAdjacencyEntriesOfVertex(
...
void multiscalelinkedcell::HangingVertexBookkeeper::beginIteration() {
  for (VertexMap::iterator p = _vertexMap.begin(); p != _vertexMap.end(); p++) {
    p->second.usedInLastTraversal = false;
  }
}

void multiscalelinkedcell::HangingVertexBookkeeper::endIteration() {
  for (VertexMap::iterator p = _vertexMap.begin(); p != _vertexMap.end();) {
...
tarch::la::Vector<TWO_POWER_D, int>
multiscalelinkedcell::HangingVertexBookkeeper::createHangingVertex(
    const tarch::la::Vector<DIMENSIONS, double>& x, int level,
  ...
  logTraceInWith6Arguments("createHangingVertex(...)", x, level,
                           fineGridPositionOfVertex, adjacencyEntries, key,
                           _vertexMap.count(key));

  if (_vertexMap.count(key) == 0) {
    HangingVertexIdentifier newHangingVertexIdentifier;
    newHangingVertexIdentifier.indicesOfAdjacentCells =
        createVertexLinkMapForNewVertex();
    newHangingVertexIdentifier.usedInLastTraversal = false;
    _vertexMap.insert(
        std::pair<tarch::la::Vector<DIMENSIONS + 1, double>,
                  HangingVertexIdentifier>(key, newHangingVertexIdentifier));
  }

  tarch::la::Vector<DIMENSIONS, int> fromCoarseGridVertex;
  tarch::la::Vector<DIMENSIONS, int>
      coarseGridVertexAdjacentCellDescriptionIndex;

  dfor2(k) for (int d = 0; d < DIMENSIONS; d++) {
  ...
  if ((_vertexMap[key].indicesOfAdjacentCells(kScalar) ==
       InvalidAdjacencyIndex) ||
      (_vertexMap[key].indicesOfAdjacentCells(kScalar) ==
    ...
    _vertexMap[key].indicesOfAdjacentCells(kScalar) = adjacencyEntries(
        ...
        peano::utils::dLinearised(coarseGridVertexAdjacentCellDescriptionIndex,
                                  2));
  }
  enddforx

      logTraceOutWith2Arguments("createHangingVertex(...)",
                                _vertexMap[key].indicesOfAdjacentCells,
                                _vertexMap[key].usedInLastTraversal);
