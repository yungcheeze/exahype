...
tarch::logging::Log exahype::mappings::VolumeIntegral::_log(
...
exahype::mappings::VolumeIntegral::VolumeIntegral(
    const VolumeIntegral& masterThread) {
  // do nothing
}

void exahype::mappings::VolumeIntegral::mergeWithWorkerThread(
    const VolumeIntegral& workerThread) {
  // do nothing
}
#endif
      ...
      for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 1; j++) {
        ...
        double* lFhi =
            &(DataHeap::getInstance()
                  .getData(cellDescription.getVolumeFlux(patchIndex))[0]
                  ._persistentRecords._u);
        double* lduh = &(DataHeap::getInstance()
                             .getData(cellDescription.getUpdate(patchIndex))[0]
                             ._persistentRecords._u);

        dg::volumeIntegral<DIMENSIONS>(lduh, lFhi, dxPatch);
      }
    }
  }
