...
tarch::logging::Log exahype::mappings::SurfaceIntegral::_log(
...
exahype::mappings::SurfaceIntegral::SurfaceIntegral(
    const SurfaceIntegral& masterThread) {
  // do nothing
}

void exahype::mappings::SurfaceIntegral::mergeWithWorkerThread(
    const SurfaceIntegral& workerThread) {
  // do nothing
}
#endif
  ...
  if (!fineGridCell.isRefined()) {
    ...
    for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 1; j++) {
      for (int i = 1; i < EXAHYPE_PATCH_SIZE_X + 1; i++) {  // loop over patches
        ...
        double* lFhLeft = &(DataHeap::getInstance()
                                .getData(cellDescription.getFluctuation(
                                    patchIndex))[dofStartIndexLeft]
                                ._persistentRecords._u);
        double* lFhRight = &(DataHeap::getInstance()
                                 .getData(cellDescription.getFluctuation(
                                     patchIndex))[dofStartIndexRight]
                                 ._persistentRecords._u);
        double* lFhFront = &(DataHeap::getInstance()
                                 .getData(cellDescription.getFluctuation(
                                     patchIndex))[dofStartIndexFront]
                                 ._persistentRecords._u);
        double* lFhBack = &(DataHeap::getInstance()
                                .getData(cellDescription.getFluctuation(
                                    patchIndex))[dofStartIndexBack]
                                ._persistentRecords._u);

        dg::surfaceIntegral(lduh, dxPatch, lFhLeft, lFhRight, lFhFront,
                            lFhBack);
      }
    }
  }
