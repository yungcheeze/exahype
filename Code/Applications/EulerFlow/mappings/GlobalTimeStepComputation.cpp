...
exahype::mappings::GlobalTimeStepComputation::GlobalTimeStepComputation()
    : _localState()  // initialises the old and current time step size to max
                     // double value
{
  // do nothing
}
  ...
  if (!fineGridCell.isRefined()) {
    ...
    for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 1; j++) {
      for (int i = 1; i < EXAHYPE_PATCH_SIZE_X + 1; i++) {  // loop over patches
        ...
        double admissiblePatchTimeStep =
            dg::stableTimeStepSize<DIMENSIONS>(luh, dxPatch, lambda);

        _localState.setTimeStepSize(
            std::min(_localState.getTimeStepSize(), admissiblePatchTimeStep));
      }
    }
  }
...
void exahype::mappings::GlobalTimeStepComputation::endIteration(
  ...
  logInfo("endIteration(...)",
          "local time step size: " << _localState.getTimeStepSize())
...
const exahype::State& exahype::mappings::GlobalTimeStepComputation::getState()
    const {
  return _localState;
}
