...
void exahype::mappings::RiemannSolver::enterCell(
  ...
  if (!fineGridCell.isRefined()) {
    ...
    // work vectors
    double QavL[sizeWithPadding(EXAHYPE_NVARS)]
        __attribute__((aligned(ALIGNMENT)));  // av: average
    double QavR[sizeWithPadding(EXAHYPE_NVARS)]
    ...
    assertion1WithExplanation(
        _localState.getOldTimeStepSize() < std::numeric_limits<double>::max(),
        _localState.getOldTimeStepSize(),
        "Old time step size was not initialised correctly!");
      ...
      for (int i = 1; i < EXAHYPE_PATCH_SIZE_X + 2; i++) {  // loop over patches
        ...
        double* QL = &(DataHeap::getInstance()
                           .getData(cellDescription.getExtrapolatedPredictor(
                               patchIndexLeft))[dofStartIndexR]
                           ._persistentRecords._u);
        double* QR = &(DataHeap::getInstance()
                           .getData(cellDescription.getExtrapolatedPredictor(
                               patchIndex))[dofStartIndexL]
                           ._persistentRecords._u);
        ...
        dg::solveRiemannProblem<DIMENSIONS>(FL, FR, QL, QR, QavL, QavR, lambdaL,
                                            lambdaR,
                                            _localState.getOldTimeStepSize(),
    ...
    for (int i = 1; i < EXAHYPE_PATCH_SIZE_X + 1; i++) {  // loop over patches
      for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 2; j++) {
        ...
        double* QL = &(DataHeap::getInstance()
                           .getData(cellDescription.getExtrapolatedPredictor(
                               patchIndexFront))[dofStartIndexR]
                           ._persistentRecords._u);
        double* QR = &(DataHeap::getInstance()
                           .getData(cellDescription.getExtrapolatedPredictor(
                               patchIndex))[dofStartIndexL]
