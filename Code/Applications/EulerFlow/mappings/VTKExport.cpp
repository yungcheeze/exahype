...
tarch::logging::Log exahype::mappings::VTKExport::_log(
    "exahype::mappings::VTKExport");

exahype::mappings::VTKExport::VTKExport()
    : _vtkWriter(0), _vertexWriter(0), _cellWriter(0), _vertexValueWriter(0) {
  // do nothing
}
...
void exahype::mappings::VTKExport::mergeWithWorkerThread(
    const VTKExport& workerThread) {}
#endif
...
void exahype::mappings::VTKExport::touchVertexFirstTime(
  ...
  if (fineGridVertex.getRefinementControl() !=
      ...
      fineGridVertex.getRefinementControl() !=
          exahype::Vertex::Records::Refining) {
  ...
  if (!fineGridCell.isRefined()) {
    ...
    for (int ii = 0; ii < basisSize; ii++) {
      for (int jj = 0; jj < basisSize; jj++) {
        ...
        uniformCoordinates[0][uniformNodeIndex] =
            (double)ii / (double)EXAHYPE_ORDER;
        uniformCoordinates[1][uniformNodeIndex] =
            (double)jj / (double)EXAHYPE_ORDER;
      }
    }
    ...
    for (int i = 1; i < EXAHYPE_PATCH_SIZE_X + 1; i++) {  // loop over patches
      for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 1; j++) {
        ...
        //          std::memset((double *) &subData[0],0,sizeof(double) *
        //          EXAHYPE_NVARS * EXAHYPE_NBASIS_POWER_DIMENSIONS);
        ...
        for (int ii = 0; ii < basisSize;
             ii++) {  // project on subgrid coordinates
          for (int jj = 0; jj < basisSize; jj++) {
            const int uniformNodeIndex = ii + basisSize * jj;

            for (int mm = 0; mm < basisSize;
                 mm++) {  // project on subgrid coordinates
              for (int nn = 0; nn < basisSize; nn++) {
                const int nodeIndex = mm + basisSize * nn;
                const int dofStartIndex = nodeIndex * nvar;

                for (int ivar = 0; ivar < nvar; ivar++) {
                  uniformDoF[ivar][uniformNodeIndex] +=
                      luh[dofStartIndex + ivar] *
                      dg::subOutputMatrix[nodeIndex][uniformNodeIndex];
                }
              }
            }
          }
        ...
        for (int ii = 0; ii < basisSize; ii++) {  // loop over dof
          for (int jj = 0; jj < basisSize; jj++) {
            ...
            geometry::mapping2d(center(0), center(1), dx, dy, dxPatch, dyPatch,
                                i, j, r, s, &x, &y);
            tarch::la::Vector<DIMENSIONS, double> currentVertexPosition(x, y);

            const int vtkNodeIndex =
                _vertexWriter->plotVertex(currentVertexPosition);
...
void exahype::mappings::VTKExport::beginIteration(exahype::State& solverState) {
  ...
  //    _cellValueWriter   =
  //    _vtkWriter->createCellDataWriter("concentration",1);

  logTraceOutWith1Argument("beginIteration(State)", solverState);
}

void exahype::mappings::VTKExport::endIteration(exahype::State& solverState) {
