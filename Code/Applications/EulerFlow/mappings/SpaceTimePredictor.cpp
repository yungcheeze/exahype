...
void exahype::mappings::SpaceTimePredictor::enterCell(
  ...
  if (!fineGridCell.isRefined()) {
    ...
    double* rhs0 = (double*)std::malloc(
        numberOfSpaceTimeDof * sizeof(double));  // todo remove all mallocs,
                                                 // maximum nvar is known at
                                                 // compile time
    double* rhs = (double*)std::malloc(
        numberOfSpaceTimeDof * sizeof(double));  // todo remove all mallocs,
                                                 // maximum nvar is known at
                                                 // compile time
    double* tmp = (double*)std::malloc(
        nvar * basisSize * sizeof(double));  // todo remove all mallocs, maximum
                                             // nvar is known at compile time
      ...
      for (int j = 1; j < EXAHYPE_PATCH_SIZE_Y + 1; j++) {
        ...
        double* lQi =
            &(DataHeap::getInstance()
                  .getData(cellDescription.getSpaceTimePredictor(patchIndex))[0]
                  ._persistentRecords._u);
        double* lFi = &(DataHeap::getInstance()
                            .getData(cellDescription.getSpaceTimeVolumeFlux(
                                patchIndex))[0]
                            ._persistentRecords._u);
        ...
        double* lQhi =
            &(DataHeap::getInstance()
                  .getData(cellDescription.getPredictor(patchIndex))[0]
                  ._persistentRecords._u);
        double* lFhi =
            &(DataHeap::getInstance()
        ...
        dg::spaceTimePredictor<2>(lQi, lFi, luh, lQhi, lFhi, lQhbnd, lFhbnd,
                                  rhs0, rhs, tmp, dxPatch,
