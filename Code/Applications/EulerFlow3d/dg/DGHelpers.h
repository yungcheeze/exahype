...
namespace exahype {
  namespace dg {
    ...
    ///////////////////////////////////////////////////////////////////////////////
    /**
     * \brief Returns the neighbour face id to a given.
     */
    inline void GetNeighborFace(const int face,int *face_neighbour) {
      // pure integer arithmetic: *face_neighbour = face + ((face+1) % 2) - ((face) % 2);
      switch (face) {
      case EXAHYPE_FACE_LEFT:
      case EXAHYPE_FACE_FRONT:
      case EXAHYPE_FACE_BOTTOM:
        *face_neighbour = face+1;
        break;
